...
import com.ery.api.service.ClientServiceApi
...
class ClientServiceImpl @Inject constructor(private val ktorClient: HttpClient): ClientServiceApi {
    ...
    override suspend fun updateClient(client: Client): Boolean {
        return try {
            ktorClient.put(HttpRoutes.CLIENTS) {
                contentType(ContentType.Application.Json)
                setBody(client)
            }
            true
    ...
    override suspend fun getClient(id: Int): Client? {
        return try {
            ktorClient.get("${HttpRoutes.CLIENTS}?") {
                parameter("id", id)
            }.body<Client>()
        } catch(e: RedirectResponseException) {
            // 3xx - responses
    ...
    override suspend fun getAllClients(): List<Client?> {
        return try {
            ktorClient.get(HttpRoutes.CLIENTS).body<List<Client>>()
        } catch(e: RedirectResponseException) {
            // 3xx - responses
            println("Error: ${e.response.status.description}")
            emptyList()
